...
The release-please workflow runs on push to `main` and maintains a release PR. When the PR merges, release-please creates the tag, which triggers the release workflow.
...
- Multi-arch container image built from a distroless static base, pushed to GHCR with both per-arch tags and a manifest list (`docker_manifests:`).
...
GoReleaser runs in a regular (non-isolated) job. It does not — and structurally cannot — produce SLSA Build Level 3 provenance on its own. That's done in a separate workflow (B4).

### B4. SLSA Build Level 3 via isolated reusable workflow

SLSA BL3 requires the signing/provenance step to run on infrastructure that the build job cannot tamper with. The canonical 2026 path (recommended by both GitHub and GoReleaser) is to split the work across two workflows:

```
.github/workflows/
  release.yml          # caller — runs on tag push, drives goreleaser
  attest.yml           # reusable (workflow_call) — runs actions/attest@v4 only
```

- **`release.yml` `build` job**: checkout, setup-go, qemu/buildx, cosign-installer, syft, GHCR login, Trivy pre-push scan, `goreleaser release --clean`. Outputs `dist/checksums.txt` and `dist/digests.txt`.
- **`release.yml` `attest-binaries` job**: calls `./.github/workflows/attest.yml` with the checksums file.
- **`release.yml` `attest-image` job**: calls `./.github/workflows/attest.yml` with the digests file.
- **`attest.yml`**: a single-step reusable workflow that runs `actions/attest@v4` against its input. Permissions: `id-token: write`, `attestations: write`, `contents: read`, `packages: write`.

The isolation between `release.yml`'s build job and `attest.yml`'s reusable execution context is what earns BL3. The build job has no access to the attestation infrastructure, and the attestation job has no user-controlled build code. `actions/attest-build-provenance` is now a thin wrapper around `actions/attest@v4` (unified v4 line) — either name works; we use `actions/attest@v4` directly.

Consumers verify with:
```
gh attestation verify --owner <owner> \
  --signer-workflow <owner>/<repo>/.github/workflows/attest.yml ./binary
...
`anchore/sbom-action/download-syft@v0` installs syft. GoReleaser's `sboms:` block runs syft against each archive and the container image, attaches the SBOM files (SPDX JSON) to the GitHub Release, and (via cosign) attaches them to the OCI image as a referrer.
...
1. **Pre-push, in the release job**: Trivy scans the locally built image with `exit-code: 1, severity: CRITICAL,HIGH`. The release fails before GoReleaser pushes if a CRITICAL or HIGH CVE is detected. Block-on-publish is the right tradeoff for a security-critical service — a release we wouldn't be willing to ship is one we shouldn't ship.
...
- **`goreleaser release --snapshot --clean`**: verifies the release config and produces unsigned local artifacts. Catches GoReleaser config errors before they hit the actual release.
...
| Job | `contents` | `pull-requests` | `packages` | `id-token` | `attestations` |
|---|---|---|---|---|---|
| `ci.yml` (lint/test/build) | read | — | — | — | — |
| `release-please.yml` | write | write | — | — | — |
| `release.yml` build | write | — | write | — | — |
| `attest.yml` reusable | read | — | write | write | write |

`id-token: write` and `attestations: write` exist only in the isolated reusable attestation workflow — that's the permission boundary that earns SLSA BL3.
